...
import heapq
from utils import Node, extract_path, g_cost
    ...
    while frontier:
        ...
        for next_state, action in puzzle.get_neighbors(node.state):
            ...
            if child_key not in reached and child_key not in frontier_states:
                frontier.append(Node(next_state, node, action, node.depth + 1))
...
def ucs(puzzle):
    ...
    frontier = []
    counter = 0
    heapq.heappush(frontier, (g_cost(start_node.state, puzzle.goal), counter, start_node))
    ...
    while frontier:
        ...
        for next_state, action in puzzle.get_neighbors(node.state):
            ...
            if key not in reached:
                ...
                counter += 1
                child = Node(next_state, node, action, node.depth + 1)
                heapq.heappush(frontier, (g_cost(next_state, puzzle.goal), counter, child))
    return None
    ...
    while frontier:
        ...
        for next_state, action in puzzle.get_neighbors(node.state):
            if not _is_cycle(node, next_state):
                children.append(Node(next_state, node, action, node.depth + 1))
